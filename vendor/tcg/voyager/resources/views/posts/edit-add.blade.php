    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            @if(isset($dataTypeContent->marca_id))
                $.post("/admin/lista_modelos", { _token: "{{ csrf_token() }}" , modelo_id:@if($dataTypeContent->modelo_id){{$dataTypeContent->modelo_id}}@else 0 @endif, elegido: {{$dataTypeContent->marca_id}} }, function(data){
                    $("#modelo_id").html(data);
                }); 
            @endif
        });
