...
use TCG\Voyager\Http\Controllers\Traits\TraitPagos;
use Mail;
use \App\Mail\interesadosresponse as consultaresponse;
...
class VoyagerBaseController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
        ...
        $payment_data = null;
        if($slug == "pagos"){
            $payment_data = $this->TRshowUser($dataTypeContent);
        }
        return Voyager::view($view, compact('dataType','payment_data', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        /**
         * Esta condicion esta hecha para cuando se resiva un formulario de respuesta
         * a una consulta en el administrador.
         */
        if($slug == "interesados"){
            $post = $data->post()->select("id","title")->first();
            if($post && !$data->answered){
                try {
                    $user = DB::table("users")->select("id","email")->where("id",$data->user_id)->first();
                    Mail::to($user->email)
                        ->send(new consultaresponse($data->mensaje, $post->title, $request->answered));
                } catch (\Throwable $th) {
                    return redirect()->back()->with([
                        'message'    => "La consulta no ha sido respondida por el siguiente motivo ".$th->getMessage() ,
                        'alert-type' => 'error',
                    ]);
                }
            }
